...
static int CeedOperatorSetup_OpenCL(CeedOperator op) {
  ...
  // Infields
  ierr = CeedOperatorSetupFields_OpenCL(qf, op, 0,
                                      impl->evecs, impl->qvecsin, 0,
                                      numinputfields, Q, numelements);
  CeedChk(ierr);

  // Outfields
  ierr = CeedOperatorSetupFields_OpenCL(qf, op, 1,
...
static int CeedOperatorApply_OpenCL(CeedOperator op, CeedVector invec,
...
int CeedOperatorCreate_OpenCL(CeedOperator op) {
  ...
  ierr = CeedSetBackendFunction(ceed, "Operator", op, "Apply",
                                CeedOperatorApply_OpenCL); CeedChk(ierr);
  ierr = CeedSetBackendFunction(ceed, "Operator", op, "Destroy",
                                CeedOperatorDestroy_OpenCL); CeedChk(ierr);
  return 0;
}

int CeedCompositeOperatorCreate_OpenCL(CeedOperator op) {
  int ierr;
  Ceed ceed;
  ierr = CeedOperatorGetCeed(op, &ceed); CeedChk(ierr);
